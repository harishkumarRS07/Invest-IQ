...
# Add project root to path (must be before backend imports)
...
def main():
    ...
    print("-" * 110)
    print(f"{'TICKER':<15} {'CURRENT':<10} {'PRED':<10} {'SIGNAL':<12} {'CONF':<8} {'RISK':<8} {'REASON'}")
    print("-" * 110)
    for p in report_data["predictions"]:
        reason = p.get('reason', '')
        # Truncate reason if too long
        if len(reason) > 40: reason = reason[:37] + "..."
        print(f"{p['ticker']:<15} {p['current']:<10.2f} {p['predicted']:<10.2f} {p['signal']:<12} {p['confidence']:<8.1%} {p['risk_level']:<8} {reason}")
    print("-" * 110)
